...
async def get_blockchain_state(full_node_client):
...
async def get_coin_record_by_name(full_node_client, name: bytes32):
    coin_records = await full_node_client.get_coin_record_by_name(name)
    return coin_records


async def get_coin_records_by_parent_id(full_node_client, parent_id: bytes32):
    coin_records = await full_node_client.get_coin_records_by_parent_ids([parent_id], include_spent_coins=True)
    return coin_records
...
async def get_coin_records_by_hint(full_node_client, hint: bytes32):
    coin_records = await full_node_client.get_coin_records_by_hint(hint, include_spent_coins=True)
    return coin_records
...
async def get_block_spends(full_node_client, height):
    block_records = await full_node_client.get_block_records(height, height + 1)
    header_hash = bytes32.from_hexstr(block_records[0]["header_hash"])
    result = await full_node_client.get_block_spends(header_hash)
    return result
